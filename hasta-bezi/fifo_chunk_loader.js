(function(){
  ...
  async function gunzipBase64(b64){
    ...
    throw new Error('Tarayıcı gzip açma desteklemiyor');
  }
  ...
  async function finishGzip(m){
    st('Yükleniyor — '+m.files.length+' parça / gzip');
    var b64='';
    for(var i=0;i<m.files.length;i++){
      var r=await fetch('./fifo_chunks/'+m.files[i]+'?v='+NO,{cache:'no-store'});
      if(!r.ok)throw new Error('chunk yok: '+m.files[i]);
      b64+=await r.text();
      st('Yükleniyor — '+(i+1)+'/'+m.files.length+' parça');
    }
    st('Açılıyor — sıkıştırılmış FIFO verisi');
    var json=await gunzipBase64(b64.replace(/\s+/g,''));
    var pack=JSON.parse(json);
    window.ALALYLI_FIFO_PACK=pack;
    var moveCount=Object.values(pack.moves||{}).reduce(function(a,b){return a+b.length},0);
    st('CANLI — '+(pack.summary||[]).length+' ürün / '+moveCount+' hareket / '+(pack.meta&&pack.meta.salesCount||0)+' satış');
    if(window.ALALYLI_FIFO_RENDER) window.ALALYLI_FIFO_RENDER();
    if(window.ALALYLI_MASTER_GUARD) window.ALALYLI_MASTER_GUARD();
  }
  function boot(){
    ...
    fetch(MANIFEST,{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('manifest yok');return r.json()}).then(function(m){
      ...
      if(m.type==='gz-base64') return finishGzip(m);
      ...
      st('BAĞLANMADI — '+(e&&e.message?e.message:'fifo_chunks/manifest.json yok. Büyük satış paketi henüz canlıya alınmadı.'));
      if(window.ALALYLI_MASTER_GUARD) window.ALALYLI_MASTER_GUARD();
    })
  }
