(function(){
  ...
  function matchDisc(name){var p=productData(name);if(p)return discountPct(p);return 0}
  function addLegend(){if(document.getElementById('alisSatisLegend'))return;var hero=document.querySelector('.hero');if(!hero)return;hero.insertAdjacentHTML('beforeend','<div id="alisSatisLegend" style="margin-top:8px"><span style="display:inline-block;background:#ffecec;border:1px solid #e6a8a8;color:#5a1717;padding:5px 9px;border-radius:8px;font-weight:900;margin-right:8px">Alış / maliyet alanları</span><span style="display:inline-block;background:#eaffef;border:1px solid #9bd9aa;color:#064b1e;padding:5px 9px;border-radius:8px;font-weight:900">Satış / liste / ciro alanları</span></div>')}
  function colorize(){document.querySelectorAll('table').forEach(function(t){var hs=Array.from(t.querySelectorAll('thead th')),lab=hs.map(function(x){return x.textContent.trim()});lab.forEach(function(name,i){var isAlis=/Alış|Maliyet|FIFO|Fark Fatura|Net\/İsk|Net Alış/.test(name);var isSatis=/Liste|Satış|Net Satış|İsk\.Dhl|İskonto|Mayıs Liste/.test(name);if(isAlis){hs[i].style.background='#ffecec';hs[i].style.color='#5a1717'}if(isSatis){hs[i].style.background='#eaffef';hs[i].style.color='#064b1e'}Array.from(t.querySelectorAll('tbody tr')).forEach(function(r){var c=r.children[i];if(!c)return;if(isAlis){c.style.background='#fff6f6';c.style.color='#3d1111'}if(isSatis){c.style.background='#f3fff6';c.style.color='#063b19'}})})})}
  function patchCols(){document.querySelectorAll('table').forEach(function(t){var heads=function(){return Array.from(t.querySelectorAll('thead th')).map(function(x){return x.textContent.trim()})};var th=Array.from(t.querySelectorAll('thead th')),lab=heads();var u=lab.indexOf('Ürün'),l=lab.indexOf('Liste'),lh=lab.indexOf('Liste Hrç');if(u<0||l<0)return;var old=lab.indexOf('Mayıs Listeye Göre İsk %');var old2=lab.indexOf('Mayıs İsk %');if(old<0&&old2>=0){th[old2].textContent='Mayıs Listeye Göre İsk %';old=old2}if(old<0){var hi=document.createElement('th');hi.textContent='Mayıs Listeye Göre İsk %';th[l].after(hi)}lab=heads();lh=lab.indexOf('Liste Hrç');var ld=lab.indexOf('Liste Dhl');if(lh>=0&&ld<0){var hd=document.createElement('th');hd.textContent='Liste Dhl';Array.from(t.querySelectorAll('thead th'))[lh].after(hd)}Array.from(t.querySelectorAll('tbody tr')).forEach(function(r){var c=Array.from(r.children),name=c[u]&&c[u].textContent,p=productData(name),disc=matchDisc(name);var current=heads(),iskIndex=current.indexOf('Mayıs Listeye Göre İsk %');if(c[iskIndex])c[iskIndex].innerHTML=yellow(m(disc),'%');current=heads();var lh2=current.indexOf('Liste Hrç'),ld2=current.indexOf('Liste Dhl'),iskD2=current.indexOf('İsk.Dhl');c=Array.from(r.children);if(p&&lh2>=0&&c[lh2]){c[lh2].textContent=m(listeHrc(p,name),4);if(ld2>=0&&c[ld2])c[ld2].innerHTML=yellow(m(listeDhl(p),4));if(iskD2>=0&&c[iskD2])c[iskD2].innerHTML=yellow(m(iskDhl(p),4));}})})}
  ...
  function panel(){var old=document.getElementById('alisKontrol3Parti');if(old)old.remove();var sec=document.getElementById('urun');if(!sec)return;var html='<div class="card" id="alisKontrol3Parti"><div class="head"><h3>Tek Mayıs Liste - İskonto Kontrolü</h3><div class="muted">Mayıs Liste tektir. İskonto yoksa oran 0,00% gösterilir. Alış alanları soft kırmızı, satış alanları soft yeşil görünür.</div></div><div class="wrap"><table><thead><tr><th>Fatura</th><th>Tarih</th><th>Ürün</th><th>Miktar</th><th>Mayıs Liste Dhl</th><th>Net/İsk.Dhl</th><th>Mayıs Listeye Göre İsk %</th></tr></thead><tbody>';
  ...
  function run(){var s=document.getElementById('stamp');if(s)s.textContent='Güncelleme No: '+NO;addLegend();patchCols();patchClassic();panel();colorize()}
