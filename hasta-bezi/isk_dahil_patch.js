(function(){
  ...
  function patchCols(){document.querySelectorAll('table').forEach(function(t){var heads=function(){return Array.from(t.querySelectorAll('thead th')).map(function(x){return x.textContent.trim()})};var th=Array.from(t.querySelectorAll('thead th')),lab=heads();var u=lab.indexOf('Ürün'),l=lab.indexOf('Liste'),lh=lab.indexOf('Liste Hrç');if(u<0||l<0)return;var old=lab.indexOf('Mayıs Listeye Göre İsk %');var old2=lab.indexOf('Mayıs İsk %');if(old<0&&old2>=0){th[old2].textContent='Mayıs Listeye Göre İsk %';old=old2}if(old<0){var hi=document.createElement('th');hi.textContent='Mayıs Listeye Göre İsk %';hi.style.cssText='font-weight:900;background:#fff3b0;color:#06142a';th[l].after(hi)}lab=heads();lh=lab.indexOf('Liste Hrç');var ld=lab.indexOf('Liste Dhl');if(lh>=0&&ld<0){var hd=document.createElement('th');hd.textContent='Liste Dhl';hd.style.cssText='font-weight:900;background:#fff3b0;color:#06142a';Array.from(t.querySelectorAll('thead th'))[lh].after(hd)}Array.from(t.querySelectorAll('tbody tr')).forEach(function(r){var c=Array.from(r.children),name=c[u]&&c[u].textContent,p=productData(name),disc=matchDisc(name);var current=heads(),iskIndex=current.indexOf('Mayıs Listeye Göre İsk %');if(c[iskIndex])c[iskIndex].innerHTML=disc==null?'—':yellow(m(disc),'%');current=heads();var lh2=current.indexOf('Liste Hrç'),ld2=current.indexOf('Liste Dhl'),iskD2=current.indexOf('İsk.Dhl');c=Array.from(r.children);if(p&&lh2>=0&&c[lh2]){c[lh2].textContent=m(listeHrc(p,name),4);if(ld2>=0&&c[ld2])c[ld2].innerHTML=yellow(m(listeDhl(p),4));if(iskD2>=0&&c[iskD2])c[iskD2].innerHTML=yellow(m(iskDhl(p),4));}})})}
