/* AperiON v69 Dashboard Signal
   ...
   This file is additive and does not change v68 approval logic by itself.
*/
(function(){
  ...
  async function readRows(){
    ...
    for(var i=0;i<tables.length;i++){
      try{
        ...
        var res = await q;
        if(!res.error && Array.isArray(res.data)) return res.data.filter(isPending);
      }catch(e){}
    }
    return [];
  }
  function ensureBox(){
    ...
    if(old) return old;
    var grid = document.getElementById('reportHubGrid');
  ...
  function goBankApproval(){
    ...
    setTimeout(function(){
      ...
      if(tab) tab.click();
      var panel = document.getElementById('fbanka');
  ...
  async function render(){
    ...
    var first = rows.slice().sort(function(a,b){return txt(b.mail_date || b.created_at || b.tarih).localeCompare(txt(a.mail_date || a.created_at || a.tarih));})[0] || {};
    var bank = first.bank_name || first.banka || 'Yapı Kredi';
    var source = first.source || first.kaynak || 'Gmail';
    var date = txt(first.mail_date || first.created_at || first.tarih).substring(0,16) || '-';
