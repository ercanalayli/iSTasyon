...
Relocated from root `DECISIONS.md` to `docs/DECISIONS.md` on 2026-07-31 as part of
the docs consolidation (56 -> 11 active files). Content is preserved verbatim;
only the numbering collision described below was fixed.

## Numbering fix (2026-07-31)

The source file contained two numbering passes that had never been reconciled:
a "newest decisions first" block (D-008 through D-017) and an older archive
block (D-001 through D-018) that reused the numbers D-008, D-009, D-010,
D-012, D-013, D-014, D-015, D-016 and D-017 for nine **different** decisions.
That is nine collisions, not just the one D-017 collision that had been
noticed earlier. No decision text was changed. Resolution: the first
occurrence of each colliding number (in the newest-first block) keeps its
original number; the second occurrence (in the archive block) is renumbered
to the next free number after the highest existing number (D-018), in the
order it appears in the file. That produces D-019 through D-027 below. The
document below preserves the original file's physical ordering, so the
numbers are not in strictly ascending order top-to-bottom — treat each
decision's number as a stable ID, not a position index.
...
**2026-07-31 guncelleme:** Bu hardening 30 Temmuz'da uygulandigi dusunulmustu, ancak tam kapanmamisti. 31 Temmuz'da yapilan canli Supabase denetiminde `ingest_mail_bank_movements`, `mark_bizimhesap_queue_processed`, `finance_calendar_log_action`, `kullanici_firma_idler`, `on_maliyet_upload`, `rls_auto_enable` fonksiyonlarinin hala anon calistirilabilir oldugu (ikisi PUBLIC rolu uzerinden miras) bulundu ve hepsi REVOKE edilip dogrulandi. Ayrica `bank_transactions` tablosunda anon SELECT (`qual: true`, herkes okuyabiliyor) ve authenticated INSERT/UPDATE (`qual/with_check: true`, sinirsiz yazma) politikalari bulundu — bunlar KASITLI olarak henuz duzeltilmedi, kullanici ile "birlikte tasarlayalim" denildi. Bkz. `docs/OPERATIONS_RULES.md` guvenlik bolumu.
...
Karar: Asagidaki dosyalar zorunlu tek kaynak kabul edilir:
...
**2026-07-31 guncelleme:** Bu bes dosya, 2026-07-31 doküman konsolidasyonuyla `docs/` altina tasindi/birlestirildi: `PROJECT_STATUS.md` ve `NEXT_TASK.md` icerigi `docs/CURRENT_STATUS.md` tarafindan supersede edildi (07-29 tarihli veriler 07-31 canli durumla degistirildi, orijinalleri `docs/archive/` icinde durur), `CHANGELOG_APERION.md` -> `docs/CHANGELOG.md` icine tasindi (bu dosya), `QA_CHECKLIST.md` maddeleri `docs/CHANGELOG.md` icine ilgili girdilerin altina "test edildi" notu olarak katlandi, `DECISIONS.md` -> bu dosyaya (`docs/DECISIONS.md`) tasindi. Yeni tek kaynak listesi `docs/START_HERE.md` icindedir.
...
**2026-07-31 guncelleme: bu karar artik gecerli degil, bkz. D-018.** Cloudflare Pages (`https://aperion-istasyon.pages.dev/`) 31 Temmuz'da canli oldugu `_redirects` + meta-refresh zinciriyle dogrulandi ve kanonik adres ilan edildi; GitHub Pages sadece yedek/preview'dur.

## D-019 Belirsiz Gelen Banka Parasi

*(Kaynakta D-008 olarak numaralanmisti; yukaridaki farkli D-008 "Kullanici Fonksiyonu Koruma Kurali" ile cakisiyordu. 2026-07-31'de D-019 olarak yeniden numaralandi, icerik degismedi.)*
...
## D-020 Tarihsel Ekstre Mutabakati ve Kaynak Hesap Koruması

*(Kaynakta D-009 olarak numaralanmisti; yukaridaki D-009 "Bilanco Veri Kaniti Kurali" ile cakisiyordu. 2026-07-31'de D-020 olarak yeniden numaralandi, icerik degismedi.)*
...
## D-021 Hattat Aylik Odeme Listesi Kaynak Kurali

*(Kaynakta D-010 olarak numaralanmisti; yukaridaki D-010 "Personal accruals are source-bound" ile cakisiyordu. 2026-07-31'de D-021 olarak yeniden numaralandi, icerik degismedi.)*
...
## D-022 Banka Kaynak Tarihi Zorunlulugu

*(Kaynakta D-012 olarak numaralanmisti; yukaridaki D-012 "Ana ekran finans karar yuzeyi" ile cakisiyordu. 2026-07-31'de D-022 olarak yeniden numaralandi, icerik degismedi. Not: kaynak dosyada bu blokta D-011 numarasi hic kullanilmamisti, D-010'dan direkt D-012'ye atlaniyordu.)*
...
## D-023 Kaynak Tarihi Duzeltme Kaniti

*(Kaynakta D-013 olarak numaralanmisti; yukaridaki D-013 "Tahsilat ve odeme kanal ayrimi" ile cakisiyordu. 2026-07-31'de D-023 olarak yeniden numaralandi, icerik degismedi.)*
...
## D-024 Apsiyon Bakiye ve Tahakkuk Ayrimi

*(Kaynakta D-014 olarak numaralanmisti; yukaridaki D-014 "Ana sayfa once finans karari" ile cakisiyordu. 2026-07-31'de D-024 olarak yeniden numaralandi, icerik degismedi.)*
...
## D-025 Apsiyon Aylik Kategori Defteri

*(Kaynakta D-015 olarak numaralanmisti; yukaridaki D-015 "Donemsel matris ve kanitli nakit dagitimi" ile cakisiyordu. 2026-07-31'de D-025 olarak yeniden numaralandi, icerik degismedi.)*
...
## D-026 Gelir Tablosu ve Bilanco Yerlesimi

*(Kaynakta D-016 olarak numaralanmisti; yukaridaki D-016 "Varsayilan finans gorunumu bugun" ile cakisiyordu. 2026-07-31'de D-026 olarak yeniden numaralandi. Not: kaynak dosyada bu karin metni yanlislikla D-017'nin ("Banka Hesap Ozeti E-postasi") altina, D-018 basligindan hemen once tek paragraf olarak dusmustu; asagida dogru basligina geri baglandi, kelimesi kelimesine korunarak.)*
...
## D-027 Banka Hesap Ozeti E-postasi Karari

*(Kaynakta D-017 olarak numaralanmisti; en yukaridaki D-017 "Moka United POS tahsilatinin banka aktarimi" ile cakisiyordu — bu, kullanicinin orijinal olarak fark ettigi collision idi. 2026-07-31'de D-027 olarak yeniden numaralandi, icerik degismedi.)*
...
**2026-07-31 confirmation:** verified live via `_redirects` -> `aperion.html` -> `aperion-ust-akil.html` redirect chain. This is the real live home screen. `aperion-home.html` / `aperion-home-v3.html` / `aperion-home-v2.html` are abandoned candidates, not live.
