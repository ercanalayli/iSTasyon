...
function money(v){const n=Number(clean(v).replace(/TL|TRY/gi,'').replace(/\./g,'').replace(',','.'));return Number.isFinite(n)?n:0}
...
function typeOf(desc,amount){const u=trUpper(desc);if(u.includes('POS')||u.includes('UYE ISYERI'))return 'pos';if(u.includes('FAST')||u.includes('EFT')||u.includes('HAVALE'))return amount>=0?'tahsilat':'odeme';if(u.includes('SGK'))return 'sgk';if(u.includes('VERGI'))return 'vergi';if(u.includes('KART'))return 'kredi_karti';if(u.includes('MASRAF')||u.includes('UCRET'))return 'banka_masrafi';return amount>=0?'tahsilat':'odeme'}
...
export function parseIsbank(text,meta={}){
  ...
  const moneyRe=/(-?\d{1,3}(?:\.\d{3})*,\d{2})\s*(?:TL|TRY)?\s+(-?\d{1,3}(?:\.\d{3})*,\d{2})\s*(?:TL|TRY)?/;
  let cur=null;
  function flush(){
    if(!cur)return;
    const joined=clean(cur.parts.join(' '));
    const mm=joined.match(moneyRe);
    if(!mm)return;
  ...
  for(const line of lines){
    ...
    if(m){flush();cur={date:m[1],time:m[2]||'',parts:[m[3]||'']}}
