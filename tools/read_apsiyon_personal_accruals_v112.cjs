...
async function main() {
  ...
  const result = {
    ...
    summary: { found: 0, ready_for_calendar_review: 0, needs_review: 0 }, accruals: [], discovered_links: [],
  };
  ...
  try {
    ...
    result.message = result.accruals.length ? 'Tahakkuk adaylari kaynak kanitiyla okundu. Finans Takvimi importu ayri onay adimidir.' : 'Tahakkuk satiri bulunamadi. Borclarim veya tahakkuk ekrani URLsi APSIYON_ACCRUALS_URL olarak ayarlanmalidir.';
    writeJson(out, result);
