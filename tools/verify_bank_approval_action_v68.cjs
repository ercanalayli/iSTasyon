...
const checks = [
  ...
  ['ledger route is visible', index.includes('bank-ledger-route') && /function\s+bankLedgerRouteHtml\s*\(/.test(index)],
  ['human confirmation meaning is visible', index.includes('bank-human-confirm') && index.includes('Onay anlamÄ±')],
  ['posting cell shows ledger route', /function\s+bankPostingCell\s*\([\s\S]*bankLedgerRouteHtml\(r\)[\s\S]*bankPostingFlowHtml\(r\)/.test(index)],
  ['morning approval cards show ledger route', /function\s+bankMorningCardHtml\s*\([\s\S]*bankLedgerRouteHtml\(r\)[\s\S]*bankPostingFlowHtml\(r\)/.test(index)],
  ['morning approval cards show posting flow', /function\s+bankMorningCardHtml\s*\([\s\S]*bankPostingFlowHtml\(r\)/.test(index)],
  ['posting flow status is visible', /function\s+bankPostingFlowHtml\s*\(/.test(index) && index.includes('AperiON onayÄ±') && index.includes('BizimHesap kuyruÄŸu') && index.includes('KayÄ±t sonucu')],
  ...
  ['dashboard stamp uses live bank approval report', index.includes('LAST_BANK_APPROVAL_STATUS?.created_at') && /function\s+refreshBuildStamp\s*\(/.test(index) && index.includes('SON RAPOR ${latestDataStamp()} Â· ${BUILD_CODE}')],
  ...
  ['home bank approval candidate focus action exists', /function\s+openBankApprovalFocus\s*\(/.test(index) && index.includes('AdayÄ± AÃ§')],
  ...
  ['home bank approval decision fields are visible', index.includes('BizimHesap hedefi') && index.includes('Banka hesabÄ±') && index.includes('Cari / karÅŸÄ± taraf') && index.includes('Kategori')],
  ['home bank approval summary stats are visible', index.includes('bank-command-status-stats') && index.includes('Taranan') && index.includes('GÃ¼venli aday') && index.includes('Ä°nceleme') && index.includes('Kuyruk') && index.includes('Rapor')],
  ['home bank approval card explains approval location', index.includes('Onay ekrandaki ilgili satÄ±rdan verilecek')],
