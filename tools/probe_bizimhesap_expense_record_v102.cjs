...
async function main() {
  ...
  try {
    ...
    const row = await page.evaluate(id => {
      ...
      const button = [...found.querySelectorAll('button,a')].find(node => /iÅŸlem|islem/i.test(node.innerText || node.value || ''));
    ...
    const clicked = await page.evaluate(id => {
      ...
      const button = found && [...found.querySelectorAll('button,a')].find(node => /iÅŸlem|islem/i.test(node.innerText || node.value || ''));
    ...
    const actions = await page.evaluate(() => [...document.querySelectorAll('a,button')]
      .filter(node => /dÃ¼zenle|duzenle|sil|detay/i.test(node.innerText || node.value || ''))
