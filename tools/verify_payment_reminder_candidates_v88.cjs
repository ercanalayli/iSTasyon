...
assert(aidat, 'BatÄ±kent aidat kartÄ± bulunamadÄ±');
assert.strictEqual(aidat.due_date, '2026-07-16');
assert.strictEqual(aidat.level, 'due_in_3');
assert.strictEqual(aidat.notify, true);
assert(sena, 'Sena Medikal kartÄ± bulunamadÄ±');
assert.strictEqual(sena.level, 'overdue');
assert.strictEqual(sena.notify, true);
assert(formatDigest(report).includes('BatÄ±kent Ercan Ev AidatÄ±'));
assert(formatDigest(report).includes('Ã¶deme emri deÄŸildir'));
