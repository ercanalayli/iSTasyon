...
const healthPage = fs.readFileSync('aperion-system-health.html', 'utf8');
...
assert(reader.includes("status: blockedSources.length ? 'degraded' : 'healthy'"));
assert(reader.includes('blocked_sources: blockedSources'));
...
assert(healthPage.includes('/api/session-bootstrap?health=1'));
assert(healthPage.includes('delete safe.device_token'));
