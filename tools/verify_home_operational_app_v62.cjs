...
const checks = [
  ...
  ['action buttons', /Gelire Ä°n[\s\S]*Gidere Ä°n[\s\S]*Banka Onay[\s\S]*Finans Merkezi/.test(html)],
  ['pipeline steps', /incomePipelineStep\('SatÄ±ÅŸ'[\s\S]*incomePipelineStep\('Gider'[\s\S]*incomePipelineStep\('Banka'[\s\S]*incomePipelineStep\('Onay'/.test(html)],
  ['operation bars', /incomeBar\('Tahsilat gerÃ§ekleÅŸme'[\s\S]*incomeBar\('Ã–deme gerÃ§ekleÅŸme'/.test(html)],
...
console.log('RESULT: OK - Ana ekran Retool/Geckoboard mantiginda operasyonel app kurgusuna geÃ§ti.');
